/**
 * This file contains typing procedures
 */
...
Blockly.StaticTyping.prototype.setBlockTypeWarning =
  function (block, blockType, varName) {
    ...
    if ((blockType == Blockly.Types.CHILD_BLOCK_MISSING) ||
      ...
      block.setWarningText('La variable ' + varName + ' a comme type "' + this.varTypeDict[varName].typeId + '" et\n' + 'ce block essaye de passer au type "' + blockType.typeName + '"!', warningLabel);
    } else {
      block.setWarningText(null, warningLabel);
    }
  };
...
Blockly.StaticTyping.prototype.setProcedureArgs = function (workspace) {
  ...
  for (var i = 0, length_ = blocks.length; i < length_; i++) {
    ...
    if (setArgsType) {
      for (const key in this.varTypeDict) {
        if (this.varTypeDict[key].typeId == 'Null') {
          this.varTypeDict[key] = Blockly.Types.NUMBER;
        }
      }
